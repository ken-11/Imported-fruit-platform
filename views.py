...
def login_(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            logging.warning(e)
        if find_user[0].isban:
            return render(request, 'login.html', {'message': "该用户已被禁用"})
...
def register_(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if a:
                con = {'result': False, 'msg': '该用户名已经存在'}
                return HTTPResponse(json.dumps(con))
                # return render(request, 'register.html', {'messageuname':
                # '该用户名已经存在'})
        except ObjectDoesNotExist as e:
            logging.warning(e)
        con = {'result': True, "msg": '注册成功'}
        return HTTPResponse(json.dumps(con))
        if request.POST.get('pwd') != request.POST.get('cpwd'):
            con = {'result': True, "msg": ''}
            return HTTPResponse(json.dumps(con))
            # return render(request, 'register.html', {'message_':
            # '两次输入的密码不一致'})
        new_user_pwd = make_password(
            request.POST.get('pwd'), auth_check, 'pbkdf2_sha1')
...
def user_info(request):
    try:
        rec_view_list = list()
        userinfo = get_object_or_404(
            UserInfo, uname=request.session.get('user_name'))
    ...
    content = {'userinfo': userinfo,
               'rec_view': rec_view_list, 'adss': adss[0]}
...
def add_ads(request):
    ...
    try:
        ...
        Address.objects.create(
            aname=addressee, address=Detailed_address, cellphone=cellphone, user=userd_)
    except ObjectDoesNotExist as e:
        logging.warning(e)
